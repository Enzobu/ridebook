---
description: Corrige un bug selon la méthode test-first (reproduire → test qui échoue → fix → vérifier). Respecte les règles projet.
argument-hint: <#numéro d'issue | description du bug>
---
...
- **Un ticket GitHub Issue** décrivant le bug avec **reproduction steps** et **comportement attendu**. Si absent, proposer `/create-ticket` d'abord.
...
6. **Corriger** le code avec l'expert approprié (`nest-expert`, `frontend-expert`, `iot-expert`).
