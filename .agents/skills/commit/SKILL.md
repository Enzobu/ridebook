---
name: commit
description: Commit et push la branche courante en respectant les Conventional Commits et les règles de sécurité (jamais sur main/dev directement). À déclencher quand l'utilisateur a fini une unité de travail et veut enregistrer + publier. Refuse --no-verify et --force par défaut.
---
...
3. **Tests et lint** : lancer `pnpm -r lint && pnpm -r test` sur la portée impactée. Si échec → **stopper** et proposer la skill `fix`.
4. **Règles projet** : vérifier rapidement :
