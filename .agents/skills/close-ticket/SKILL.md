---
name: close-ticket
description: Ferme un ticket GitHub Issue après vérification complète de la Definition of Done. Refuse la fermeture si la DoD n'est pas respectée. À déclencher quand l'utilisateur considère un ticket terminé et veut le fermer officiellement.
---
...
4. **Si un seul point échoue** :
   ...
   - Proposer la suite d'actions (skill `fix` sur un test manquant, mise à jour de doc, etc.).
5. **Si tout est vert** :
   ...
   - Fermer l'issue avec `gh issue close <id> --comment "<message>" --reason completed`.
