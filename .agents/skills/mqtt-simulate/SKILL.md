---
name: mqtt-simulate
description: Injecte des relevés de température/humidité factices dans le broker MQTT local pour tester backend-pays sans firmware. À déclencher quand l'utilisateur veut simuler des mesures IoT ou déclencher une alerte par anomalie. Accepte <country: BR|EC|CO> [count=10] [--anomaly].
---
...
2. Récupère les seuils du pays depuis `packages/contracts/src/country.ts`.
...
- `mqtt-simulate CO 5 --anomaly` — 5 relevés hors plage pour la Colombie.
